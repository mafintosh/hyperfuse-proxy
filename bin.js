...
if (!process.argv[2]) {
  console.error('Usage: hyperfuse-proxy folder')
  process.exit(1)
}

var from = resolve(process.argv[2])
var to

var stream = hyperfuse({
  ...
  readlink: function (path, cb) {
    ...
    fs.readlink(join(from, path), function (err, link) {
      ...
      if (link === from || link.indexOf(from + '/') === 0) link = link.replace(from, to)
      cb(0, link)
    })
  },
